...
Map::Map(double lat, double lon, int level, int width, int height)
{
    _width = width;
    _height = height;
    _zoomlevel = level;
...
void Map::_downloadTile(TileIndex tempTileIndex)
{
    ...
    QString currentWorkingPath = QDir::currentPath();
    QString cachePath = currentWorkingPath + "/mapcache.txt";
...
void Map::_grab_tile(double lat, double lon, int zoom, QString maptype)
{
    ...
    QString urlstr = QString("http://maps.googleapis.com/maps/api/staticmap?center=%1,%2&zoom=%3&maptype=hybrid&size=640x640&format=jpg&key=").arg(strLat, strLon, strZoomLevel);
    ...
    QString filepath = QString(_cachepath + "%1_%2_%3_hybrid_640_640.jpg").arg(strLat, strLon, strZoomLevel);
